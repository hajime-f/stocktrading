...
モデルが出力する予測値（0〜1の値をとる）に基づいて、各銘柄をラベル0または1に振り分けます。具体的には、0.7 を下限の閾値として、2000 銘柄を最大 50 銘柄に絞り込みます。
0.7 を下限とするため、もちろん、場合によっては選抜された銘柄の数が 50 に達さないことがありますし、0.7 より高い閾値のまま 50 銘柄に達することもあります。

私の経験上、50 に達さない場合（例えば、10 銘柄ほどしか残らない場合など）は、翌日は下げ相場になることが多く、閾値が高いまま 50 に達する場合は、上げ相場になることが多いようです。

なお、絞り込む数を最大 50 とするのは、kabu ステーション API の利用制限（取引対象となる「登録銘柄」は 50 銘柄以下）があるためです。
