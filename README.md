...
モデルが出力する予測値（0〜1の値をとる）を、ラベル0または1に振り分けるための閾値を調整することで、50 銘柄ほどに絞り込みます。

なお、絞り込む数を 50 にするのは、kabuステーションAPIの利用制限（取引対象となる「登録銘柄」は50銘柄以下）があるためです。
